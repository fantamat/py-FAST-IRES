...
# Placeholder for readlocs, Find_Patch_V2
...
def Spike_Peak_Solution():
    ...
    SPK = 'Spike_I'
    Spk_Folder = f'../Spikes/{SPK}'
    os.chdir(Spk_Folder)
    Data_Avg = sio.loadmat('Data_Avg.mat')['Data_Avg']
    TBF = sio.loadmat('TBF.mat')['TBF']
    os.chdir('../')
    Phi = Data_Avg
    Phi_noisy = Phi.copy()
    Number_sensor, Number_Source = Phi_noisy.shape
    Num_TBF = TBF.shape[0]
    Res_Folder = f'../Results/{SPK}'
    os.chdir(Res_Folder)
    J_sol = sio.loadmat('J_sol_1st.mat')['J_sol']
    TBF_1st = sio.loadmat('TBF_1st.mat')['TBF']
    ...
    J_abbas_2 = J_abbas_1 ** 2 * np.var(TBF[:, 249:350], axis=1)
    J_init = J_abbas_2
    ...
    sio.savemat('J_sol_Spk_Seg.mat', {'J_seg': J_seg})
    sio.savemat('J_sol_Spk.mat', {'J_abbas_2': J_abbas_2})
    os.chdir('../Codes')
